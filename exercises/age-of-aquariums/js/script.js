...
//create spirit
for (let i = 0; i < spiritsArraySize; i++){// go through the spirits array and create spiritsArraySize number of object spirit
  spirits[i] = createSpirit(random(0,width), random(0,height), random(0,255));// each spirit has a random x,y
...
function createSpirit(x,y){// implement the random x,y from setup
  let spirit = {
    x: x,
    y: y,
    size:68,
...
function colorizeSpirit(spirit){
  ...
  if (round === 1){// this is the round where the guide is created
    ...
    spirit.color.b =200;
    spirit.color.g =150;
    if (spirit.secret === secrets[4]){// the fifth secret plays a different sound
      spirit.song = secretSong2;
    }
    else {
      spirit.song = secretSong;}// the four others play the same twirping sparrow
    theGuide.secret = spirit.secret;
    theGuide.song = spirit.song;

  }
  if (round === 2){// round 2 then happens 50% of the time
  ...
  if (round === 3){// round 3 then happens the other 50% of the time
...
if (state===`pause`){
  ...
  if (timer === 75){// when timer reaches 75 frames
...
if (state === `title`){
  ...
  if (startTimer === 1){// when startTimer switch is on
    timer++;}// timer counts up
  if (timer < 100){// when timer reaches 100 frames
    titleLine1();// displays welcome message
  }
  else if (timer >= 83 && timer < 320){// when timer reaches 83 frames
    titleLine2();// announces the guide and its secret
  }
  else if (timer === 320){// when timer reaches 320 frames
      state = `spirits state`;// state changes to `spirits state`
  }
}
// the Spirits State
if (state === `spirits state`){
  for (let i = 0; i < spirits.length; i++){// go through the spirits array and check each spirit
    ...
    if (clickSpirit >= 3){// if the user has clicked the wrong spirits 3 times
...
if (state === `secret eludes`){
  ...
  timer++;// timer counts up
  if (timer === 170){// when timer reaches 170 frames
...
if (state === `caughtSecret`){
  ...
  timer++;// timer counts up
  if (timer === 300){// when timer reaches 300 frames
...
function mouseClicked(){
  if (state === `title`){// mouseClicked in `title` state
    let d = dist(mouseX, mouseY, cursor.x, cursor.y);// check distance between mouse and cursor
    if (d < cursor.width/2 + 10){// if mouse is over the Hermes staff when clicked
      playTitleSong = 1;// bell sound
      gameStarts++;// game starts engine switch
      startGame();}// engine is activated
    }

  else if (state === `spirits state`){// mouseClicked in `spirits state`
    for(let i = 0; i<spirits.length;i++){// go through the spirits array
      ...
      if (d < cursor.width/5 + spirit.size/2){// if the cursor is above the spirit when clicked
        ...
        if (spirit.secret!=undefined){// if the spirit clicked is the Guide, then its secret
  ...
  else if (state === `caughtSecret`){// in the caught secret state
    if (canClickEnd===1){// when timer reaches 300 frames
...
function startGame(){// when mouse is clicked in title state, puts game into motion
  if (gameStarts === 1){// gameStarts is activated
    noCursor();// the computer cursor disappears (into the staff)
    if (playTitleSong === 1){// because playTitleSong has been activated by mouse click
    titleSong.play();// bell sound plays
    playTitleSong =0;}// but only once
