class Player {
  ...
  handleInput() {
    // handles player input to make player avatar move

    if (this.wallCollision === true) {
      // if player is colliding with a barrier
      // do not handle input!
    } else if (this.wallCollision === false) {
      // if player is not colliding with a barrier
  ...
  displayInventory() {
    // display item in inventory over player avatar
    for (let i = 0; i < this.inventory.length; i++) {
      // look through inventory
      push();
      imageMode(CENTER);
      image(
        this.inventory[i].image,
        this.x + 3,
        this.y - 20,
        this.inventory[i].width,
        this.inventory[i].height
      );
      pop();
    }
  }
  ...
  checkTrade(npc, npcDesiredItem, npcHoldingItem) {
    ...
    for (let i = 0; i < this.inventory.length; i++) {
      ...
      if (this.checkedItem.name === undefined) {
        // when undefined === undefined strange things happen
        // so I made it do nothing
      } else if (this.checkedItem.name === npcDesiredItem) {
        // if the item in the inventory's name
  ...
  barriers(map) {
    // implement game barriers onto the player
